...
mod Create {
    ...
    #[event]
    use dojo examples::events::{Event, ObjectData, GameState};
    ...
    fn execute(ctx: Context, start_time: u64, turns_remaining: usize) {
        ...
        set !(
            ...
            (
                Object {
                    game_id:game_id, player_id: ctx.origin, object_id: 'Painting',
                    description: 'An intriguing painting.'
                },

                Object {
                    game_id:game_id, player_id: ctx.origin, object_id: 'Foto',
                    description: 'An egyptian cat.'
                },

                Object {
                    game_id:game_id, player_id: ctx.origin, object_id: 'Strange Amulet',
                    description: 'Until Tomorrow'
                },

                Object {
                    game_id:game_id, player_id: ctx.origin, object_id: 'Book',
                    description: '1984'
                },                
            )
        );

        emit! (ctx.world, GameState {game_state: 'Game Initilized'});
